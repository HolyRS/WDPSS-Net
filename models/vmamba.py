...
class GSS2D(nn.Module):
    ...
    def forward(self, feat_2d, ys_4chw,fdf,lambda_freq=0.5):
        ...
        fdf=fdf.permute(0,2,1,3,4)
        G = g4 + lambda_freq * fdf
        G = G / (G.sum(dim=1, keepdim=True) + 1e-6)

        # 仅在可视化脚本主动开启时保存门控，不影响正常训练和推理
        if getattr(self, "capture_gate", False):
            if not hasattr(self, "gate_records"):
                self.gate_records = []

            self.gate_records.append({
                "final_gate": G.detach().float().cpu(),
                "spatial_gate": g4.detach().float().cpu(),
                "frequency_gate": fdf.detach().float().cpu(),
            })

        # --- 对 ys_4chw 进行方向加权 ---wo
        gated = ys_4chw * (1 + 1 * (G - 0.25))
...
class SS2D(nn.Module):
    ...
    # only used to run previous version
    def forwardv0(self, fdf,x: torch.Tensor, SelectiveScan=SelectiveScanMamba, seq=False, force_fp32=True, **kwargs):
        x = self.in_proj(x)
    ...
    def forward_corev2(self, x: torch.Tensor,fdf=None, cross_selective_scan=cross_selective_scan, **kwargs):
        ...
        return cross_selective_scan(
            x,fdf, x_proj_weight, None, dt_projs_weight, dt_projs_bias,
    ...
    def forwardv2(self,x: torch.Tensor,fdf=None, **kwargs):
    ...
    def forwardxv(self, x: torch.Tensor, mode="xv1a", omul=False, **kwargs):
        ...
        def selective_scan(u, delta, A, B, C, D, delta_bias, delta_softplus,fdf):
            return SelectiveScanOflex.apply(u, delta, A, B, C, D, delta_bias, delta_softplus, 1, 1, True,fdf)
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor,fdf=None):
        if self.ssm_branch:
            if self.post_norm:
                x = input + self.drop_path(self.norm(self.op(input,fdf)))
            else:
                x = input + self.drop_path(self.op(self.norm(input),fdf))
        if self.mlp_branch:
            if self.post_norm:
    ...
    def forward(self, input: torch.Tensor,fdf=None):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input,fdf)
        else:
            return self._forward(input,fdf)
