...
class FrequencyModule(nn.Module):
    ...
    def forward(self, F):
        ...
        # Step 1: 双时相特征分别做 DWT
        # -------------------------
        HH, HL, LH, LL = self.dwt(F)

        # G1, G2: [B, 4C, H/2, W/2]
        #G = self._pack_subbands(HH, HL, LH, LL)

        # -------------------------
        # Step 2: G进入 SR 模块
        # -------------------------
        I = self.sr(HH, HL, LH, LL)   # [B, 4C, H/2, W/2]


        # -------------------------
        # Step 3: I经过 3x3 模块
        # -------------------------
        I = self.pre_idwt_refine(I)

        # -------------------------
        # Step 5: I经过 IDWT
        # -------------------------
        Y = self.idwt(I)   # [B, C, H, W]

        FDF = self.freq_gate(HH, HL, LH, target_size=(H,W))
...
class GSS2D(nn.Module):
    ...
    def __init__(self, in_channels, kappa=2.0, conf_alpha=0.75, use_confidence=True,lambda_freq=0.5,
        gate_scale=1.0,
        ...
        # ========== 1) 频率模块：内部生成 FDF 和 Y ==========
        self.freq_module = FrequencyModule(channels=in_channels)
        # ========== 2) feat_2d 与 Y 融合后降回原通道 ==========
        # 这里采用 concat -> 1x1 压缩，最符合你说的“有机结合并降维到原始尺寸”
        ...
        if use_learnable_residual:
            self.alpha_gate = nn.Parameter(torch.tensor(1.0))  # 门控分支
            self.alpha_y = nn.Parameter(torch.tensor(1.0))  # 频率特征分支
        else:
            self.register_buffer("alpha_gate", torch.tensor(1.0))
            self.register_buffer("alpha_y", torch.tensor(1.0))

    def forward(self, feat_2d, ys_4chw):
        B, C, H, W = feat_2d.shape

        # Step 1: 由原始特征图内部生成频率方向场 FDF 和频率增强特征 Y
        # =========================================================
        fdf, Y = self.freq_module(feat_2d)
        # 期望:
        #   fdf: (B, C, 4, H, W) 或 (B, 4, C, H, W)
        #   Y:   (B, C, H, W)

        # 如果 fdf 是 (B, C, 4, H, W)，转成 (B, 4, C, H, W)
        if fdf.dim() == 5 and fdf.shape[1] == C and fdf.shape[2] == 4:
            fdf = fdf.permute(0, 2, 1, 3, 4)  # -> (B, 4, C, H, W)

        # Step 2: feat_2d 与 Y 融合，再降维回原始通道数
        # =========================================================
        fused_feat = torch.cat([feat_2d, Y], dim=1)  # (B, 2C, H, W)
        fused_feat = self.fuse_reduce(fused_feat)  # (B, C, H, W)

        # --- 预测每个 channel 的二维方向向量 (B, 2*C, H, W) ---
        uv = self.dir_head(fused_feat)  # (B, 2*C, H, W)
        ...
        G = g4 + self.lambda_freq * fdf
        G = G / (G.sum(dim=1, keepdim=True) + 1e-6)

        # --- 对 ys_4chw 进行方向加权 ---wo
        gated = ys_4chw * (1 + self.gate_scale * (G - 0.25))

        # Y: (B, C, H, W) -> (B, 1, C, H, W) -> broadcast 到 4 个方向
        Y_expand = Y.unsqueeze(1)  # (B, 1, C, H, W)

        # 推荐更稳的写法：保留原始 ys，再叠加门控增量和频率增量
        #out = ys_4chw + self.alpha_gate * (gated - ys_4chw) + self.alpha_y * Y_expand
        out = torch.lerp(ys_4chw, gated, self.alpha_gate)
        out = out + self.alpha_y * Y_expand
        return out
